...
""",
    ...
    cfg = parse_config_file(cfg_path)
    assert cfg.collate_start == "2024-01-01"
    assert cfg.collate_end == "2024-12-31"
    assert cfg.threshold == 1.5
    assert cfg.min_samples == 20
    assert cfg.start == "2024-01-01"
    assert cfg.end == "2024-06-30"
...
""",
    ...
    assert repo.collate_start == "2024-06-01"
...
def test_resolve_applies_global_defaults() -> None:
    ...
    assert resolved.collate_start == "2024-01-01"
    assert resolved.collate_end == "2024-12-31"
    assert resolved.threshold == 1.5
    assert resolved.min_samples == 20
    assert resolved.start == "2024-01-01"
    assert resolved.end == "2024-06-30"


def test_resolve_repo_value_takes_precedence() -> None:
    """Per-repository values override global defaults during resolve()."""
    multi = MultiRepoConfig(collate_start="2024-01-01", threshold=2.0, min_samples=30)
    repo = RepoConfig(name="owner/repo", collate_start="2024-06-01", threshold=3.5)
    resolved = multi.resolve(repo)

    assert resolved.collate_start == "2024-06-01"
...
def test_resolve_none_global_not_overridden_by_repo_value() -> None:
    ...
    multi = MultiRepoConfig()  # collate_start=None by default
    repo = RepoConfig(name="owner/repo", collate_start="2024-03-01")
    resolved = multi.resolve(repo)
    assert resolved.collate_start == "2024-03-01"
...
def test_resolve_both_none_stays_none() -> None:
    ...
    repo = RepoConfig(name="owner/repo")
    resolved = multi.resolve(repo)
    assert resolved.collate_start is None
